...
;
	...
	const auto equals = [](lua_State *L) {
		...
		if (lua_type(L, 2) == LUA_TNUMBER) {
			uint32_t b = static_cast<uint32_t>(luaL_checkinteger(L, 2));
			
			bool res = (
				a.r == static_cast<uint8_t>(b & 0xFF) &&
				a.g == static_cast<uint8_t>((b >> 8) & 0xFF) &&
				a.b == static_cast<uint8_t>((b >> 16) & 0xFF),
				a.a == static_cast<uint8_t>((b >> 24) & 0xFF)
			);
