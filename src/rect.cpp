...
static constexpr const char *META = "rect";
...
Rect Rect::operator+(Rect const &v) const {
	auto ba = xsimd::load_aligned<xsimd::sse2>(_data);
	auto bb = xsimd::load_aligned<xsimd::sse2>(v._data);
...
void LuaRuntime::_register_rectangle() {
	...
	const auto read = [](lua_State *L) {
		...
		else if (std::strcmp(field, "width") == 0) lua_pushnumber(L, p->GetWidth());
		else if (std::strcmp(field, "height") == 0) lua_pushnumber(L, p->GetHeight());
	...
	const auto add = [](lua_State *L) {
		...
		if ((b = luaL_testudata(L, 2, "point")) != nullptr) {
			Vector2 *p = static_cast<Vector2 *>(b);
			Rect *udata = static_cast<Rect *>(lua_newuserdata(L, sizeof(Rect)));

			udata->_left = a._left + p->x;
			udata->_top = a._top + p->y;
			udata->_right = a._right + p->x;
			udata->_bottom = a._bottom + p->y;
		...
		Rect bb = *static_cast<Rect *>(luaL_checkudata(L, 2, META));
