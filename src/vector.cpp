...
static constexpr const char *META = "vector";

namespace Orbit::Lua {

float Vector::DistanceFrom(Vector const &v) const {
	auto a = xsimd::load_aligned<xsimd::sse2>(_data);
	auto b = xsimd::load_aligned<xsimd::sse2>(v._data);
...
Vector Vector::LerpTo(Vector const &v, float t) const {
	auto a = xsimd::load_aligned<xsimd::sse2>(_data);
	auto b = xsimd::load_aligned<xsimd::sse2>(v._data);

	auto simd_t = xsimd::broadcast<float, xsimd::sse2>(t);
