...
CastMember::CastMember(const std::filesystem::path &path, const std::string &lib) : path(path), library(lib) {
...
void CastLib::load_members(const std::filesystem::path &dir) {
    ...
    for (auto &entry : std::filesystem::directory_iterator(dir)) {
        ...
        auto ename_string = path.stem().string();
        const char *ename = ename_string.c_str();

        if (std::strncmp(name, ename, std::strlen(name))) continue;

        CastMember member(path, _name);
