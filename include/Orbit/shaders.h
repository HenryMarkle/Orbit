...
struct InvbCopyPixelsShader {
    ...
    inline void prepare(
        ...
        if (vflip) {
            // When vflipping, mirror the rectangle's position vertically within the texture
            float flipped_y = t1.height - src.y - src.height;  // Mirror the rectangle position
            
            src_coords[0] = src.x / t1.width;                          // left (x min) - unchanged
            src_coords[1] = flipped_y / t1.height;                     // top (y min) - mirrored position
            src_coords[2] = (src.x + src.width) / t1.width;            // right (x max) - unchanged
            src_coords[3] = (flipped_y + src.height) / t1.height;      // bottom (y max) - mirrored position
        } else {
            // Normal case
            src_coords[0] = src.x / t1.width;                          // left (x min)
            src_coords[1] = src.y / t1.height;                         // top (y min)
            src_coords[2] = (src.x + src.width) / t1.width;            // right (x max)
            src_coords[3] = (src.y + src.height) / t1.height;          // bottom (y max)
        }
...
struct InvbBevelShader {

    Shader shader;
    int texture1_loc;
    int texture1_size_loc;
    int vflip_loc;
    int vertices_loc;
    int src_coords_loc;
    int thickness_loc;

    inline Shader operator=(const InvbBevelShader &s) const { return s.shader; }
    inline void prepare(
        const Texture2D &t1, 
        const Rectangle &src, 
        const Vector2 q[4],
        ...
        SetShaderValueV(shader, texture1_size_loc, &t1s, SHADER_UNIFORM_VEC2, 1);
        SetShaderValue(shader, vflip_loc, &vflip, SHADER_UNIFORM_INT);
        SetShaderValue(shader, thickness_loc, &thickness, SHADER_UNIFORM_INT);
    }

    inline InvbBevelShader &operator=(const InvbBevelShader &) = delete;

    InvbBevelShader(const InvbBevelShader &) = delete;
    
    InvbBevelShader();

    inline ~InvbBevelShader() { UnloadShader(shader); }

};

struct SoftPropShader {

    Shader shader;
    int texture_loc;
    int texture_size_loc;
    int vflip_loc;
    int effect_color_loc;
    int self_shade_loc;
    int smooth_shading_loc;
    int depth_affecthilites_loc;
    int highlight_border_loc;
    int shadow_border_loc;
    int contour_exp_loc;
    int depth_loc;
    ...
    inline void prepare(
        ...
        int self_shade,
        int smooth_shading,
        int depth_affecthilites,
        int highlight_border,
        int shadow_border,
        bool vflip = false
    ) const { 
        SetShaderValueTexture(shader, texture_loc, t1);

        Vector2 t1s = Vector2{(float)t1.width, (float)t1.height};

        SetShaderValueV(shader, texture_size_loc, &t1s, SHADER_UNIFORM_VEC2, 1);

        SetShaderValue(shader, vflip_loc, &vflip, SHADER_UNIFORM_INT);
        SetShaderValue(shader, effect_color_loc, &effec_color, SHADER_UNIFORM_INT);
        SetShaderValue(shader, self_shade_loc, &self_shade, SHADER_UNIFORM_INT);
        SetShaderValue(shader, smooth_shading_loc, &smooth_shading, SHADER_UNIFORM_INT);
        SetShaderValue(shader, depth_affecthilites_loc, &depth_affecthilites, SHADER_UNIFORM_INT);
        SetShaderValue(shader, highlight_border_loc, &highlight_border, SHADER_UNIFORM_INT);
        SetShaderValue(shader, shadow_border_loc, &shadow_border, SHADER_UNIFORM_INT);
        SetShaderValue(shader, depth_loc, &depth, SHADER_UNIFORM_FLOAT);
    }

    inline SoftPropShader &operator=(const SoftPropShader &) = delete;

    SoftPropShader(const SoftPropShader &) = delete;
    
    SoftPropShader();

    inline ~SoftPropShader() { UnloadShader(shader); }
...
struct BevelShader {
    ...
    int texture_loc, texture_size_loc, thickness_loc, vflip_loc;

    inline Shader operator=(const BevelShader &s) const { return s.shader; }
    inline void prepare(
        const Texture &t,
        int thickness,
        bool vflip = false
        ...
        Vector2 size = { (float)t.width, (float)t.height };
        SetShaderValueV(shader, texture_size_loc, &size, SHADER_UNIFORM_VEC2, 1);
    }

    inline BevelShader &operator=(const BevelShader &) = delete;
    inline BevelShader &operator=(BevelShader &&other) noexcept {
        if (this == &other) return *this;

        shader = other.shader;
        texture_loc = other.texture_loc;
        thickness_loc = other.thickness_loc;
        vflip_loc = other.vflip_loc;

        other.shader = Shader{0};
        other.texture_loc = 0;
        other.thickness_loc = 0;
        other.vflip_loc = 0;

        return *this;
    }

    BevelShader(const BevelShader &) = delete;
    inline BevelShader(BevelShader &&other) noexcept {
        shader = other.shader;
        texture_loc = other.texture_loc;
        thickness_loc = other.thickness_loc;
        vflip_loc = other.vflip_loc;
    ...
    inline ~BevelShader() { UnloadShader(shader); }
};
...
struct Shaders {
    ...
    SoftPropShader soft_prop;
    InvbBevelShader invb_bevel;
};
